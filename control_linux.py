...
def connected_linux(comm, host=hostname, por=port, user=username, pawd=password):
    try:
        ...
        ssh_stdin, ssh_stdout, ssh_stderr = ssh.exec_command(command=comm, timeout=3)
        ...
        return "命令执行完毕，但可能有异常"

if __name__ == '__main__':

    # returl = connected_linux(comm="systemctl status mysqld.service")
    returl = connected_linux(comm="sh /www/wdlinux/wdcp/wdcp.sh start")
    print(returl)